...
async function transfer(
  ...
  try {
    ...
    if (senderBalance < amountInWei) {
      const formattedBalance = formatUnits(senderBalance, decimals);
      throw new Error(
        `余额不足！当前余额: ${formattedBalance} ${symbol}，尝试转账: ${amount} ${symbol}`
      );
    }
    ...
    if (receipt.status === "success") {
      ...
      console.log("\n💰 余额更新:");
      console.log(
        `发送方余额: ${formatUnits(newSenderBalance, decimals)} ${symbol}`
      );
      console.log(
        `接收方余额: ${formatUnits(receiverBalance, decimals)} ${symbol}`
      );
    } else {
      console.log("\n❌ 交易失败");
    }
...
async function main() {
  ...
  if (!(contractAddress && toAddress && amount)) {
    ...
    console.log("\n使用方法:");
    console.log(
      "  pnpm hardhat run scripts/transfer.ts --network sepolia [合约地址] [接收地址] [转账金额]"
    );
    console.log("\n示例:");
    console.log(
      "  pnpm hardhat run scripts/transfer.ts --network sepolia 0x2A82d3eB8b608F0CE885C75ac96C045e90c80d95 0x742d35Cc6634C0532925a3b844Bc9e7595f0bEb 100"
    );
  ...
  if (Number.isNaN(amountNum) || amountNum <= 0) {
    console.error("❌ 错误: 转账金额必须是大于 0 的数字");
    process.exit(1);
  }

  await transfer(contractAddress, toAddress, amount);
}
